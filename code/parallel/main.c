...
int main()
{
    ...
    // Get image dimensions, all processes must have that.
    int width, height;
    ...
    if (!world_rank)
    {
        ...
        for (int worker_rank = 1; worker_rank < world_size; worker_rank++)
        {
            ...
            // Send the tile height to my worker, so they know the row size.
            MPI_Send(&end_height, 1, MPI_INT, worker_rank, WORK_TAG, MPI_COMM_WORLD);
    ...
    else
    {
        ...
        // Retrieving amount tile height which is row for worker.
        int row;
        // Receiving the row amount from master.
        MPI_Recv(&row, 1, MPI_INT, 0, WORK_TAG, MPI_COMM_WORLD, &status);
        // // printf("rows: %d\n", row);
